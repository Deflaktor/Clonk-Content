...
public Activate:
 Sound("Magic1");
  DoEnergy(+100,Par(0));
  while( SetVar(0,FindObject(0,-50000,-50000,100000,100000,OCF_CrewMember(),0,0,NoContainer(),Var())))
       if(!Hostileto(GetOwner(Var()),GetOwner(Par())))
          Heal(Var());

while(SetVar(0,FindObject(HZA5,-50000,-50000,100000,100000,0,0,0,NoContainer(),Var())))
       if(!Hostileto(GetOwner(Var()),GetOwner(Par())))
		 gegengift(Var());
