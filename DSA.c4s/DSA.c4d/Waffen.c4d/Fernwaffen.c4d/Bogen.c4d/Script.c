...
Fire:
  ...
  SetLocal(2,1,Var(0));
  while ( Not(Equal(GetID(Contents(0,Contained())),BO3A)) )  if (Not(ShiftContents(Contained())))   return(0);
  ...
  SetVar(3, Sum( Div( Mul( GetPhase(Contained()), 6), 2), 2 ) );
  if ( Equal(GetDir(Contained()),DIR_Left()) ) SetVar(3, Mul(Var(3),-1) );
  SetVar(2, Div( Mul( Sum(GetPhase(Contained()),-3), 6), 2) );
