...
local weapon, distance, distanceI;
...
private HasWeapon:
  ...
  while(Var(0)=Contents(Var(2)++)) {
    if(Var(0)->~IsWeapon())
      if(Var(0)->~NeedsAmmo()) {
        if(Var(0)->~Ammo()>0||Var(0)->~ExtractAmmoPacket()!=-1)
          return(2);
        else
          needsAmmo = 1;
      } else
...
IsMarkedCantGet:
  var i = GetEffectCount(0, Par(0));
  while(i--)
    if(GetEffect("CantGet", Par(0), i, 4)==this())
      return(1);
  return(0);
...
MarkCantGet:
  return(AddEffect("CantGet", Par(0),10,30,this()));

//Clonk Nukem
Search4Weapons:
  while(SetVar(0,FindObject(0,0,0,-1,-1,0,0,0,0,Var(0)))) {
    if(ObjectCall(Var(0), "IsWeapon")) {
      if(!IsMarkedCantGet(Var(0)))
        if(WeaponFree(Var(0))) {
          distance = ObjectDistance(Var(0));
          AddCommand( this(), "Call", this(), 0,0,Var(0),0, "GetItem" );
          AddCommand(this(), "Get", Var(0),0,0,0, 200 );
          return(target=Var(0));
        }
...
Search4Ammo:
  ...
  while(Var(0)=Contents(Var(2)++)) 
    if(Var(0)->~IsWeapon())
      if(Var(0)->~NeedsAmmo())
        if(Var(0)->~ExtractAmmoPacket()==-1) {
          for(var i=0;i<=9;i++) {
            id = Var(0)->~GetAmmoPacketIDPerIndex(i);
            var tmpObj = FindObject(id,0,0,-1,-1);
			if(tmpObj)
            if(WeaponFree(tmpObj)) {
              var tmpDist = ObjectDistance(tmpObj);
              if(tmpDist < dist) {
                dist = tmpDist;
                obj = tmpObj;	  
              }
          ...
          distance = ObjectDistance(obj);
          AddCommand( this(), "Call", this(), 0,obj,Var(0),0, "LoadWeapon" );
          AddCommand( this(), "Get", obj,0,0,0, 0);
          return(target=obj);
        }
  return(0);
  
// Clonk-Karl
protected func FxCantGetTimer(pTarget, iEffectNumber, iEffectTime) {
  if(Inside(GetY(), GetY(pTarget)-20, GetY(pTarget)+20))
    if(PathFree(GetX(),GetY(),GetX(pTarget),GetY(pTarget))) {
      distance = ObjectDistance(pTarget);
      AddCommand(this(), "Get", pTarget);
      target = pTarget;
      return(-1);
    }
...
IsEnemy:
  if(!target) return(0);
  if(target->~IsWeapon()||target->~IsAmmo())
    return(0);
...
protected Activity:
  ...
  if(target)
    if(!GetAlive(target)&&!(target->~IsWeapon()||target->~IsAmmo()))
      target=0;
 
  // Je nach Schwierigkeitsgrad abbrechen
  var hasWeapon = HasWeapon();
  var needsAmmo = NeedsAmmo();
  ...
  if(target) 
    if(target->~IsWeapon()||target->~IsAmmo()) {
      if(!(GetCommand() S= "Wait"))
        distanceI++;
      if(distanceI>20) {
        distanceI = 0;
        var newDist = ObjectDistance(target);
        if(Abs(newDist-distance)<10) { 
          MarkCantGet(target);
          target = 0;
 ...
 if(!(GetAction() S= "Push"))
  ...
  if(IsEnemy(target)) {
    if(lastSightTime > 10) {
      target = 0;
	  lastSightTime = 0;
    } else if(PathFree(GetX(),GetY(),GetX(target),GetY(target))) {
  ...
  if(!Global(6)) { 
    if(!target) {
      if(needsAmmo) Search4Ammo();
      else Search4Weapons();
    } else if(IsEnemy(target)) {
      if(hasWeapon==1) Search4Ammo();
      else if(hasWeapon==0) Search4Weapons();
    }
  }
  //Log(Format("Target: %i", GetID(target)));
...
Help1:
  if(Contained(Par(0))==this())
    while(Not(Equal(ID(Par(0)),ID(Contents(0))))) ShiftContents();
  return(1);
...
ChooseWeapon:
  ...
  mostValuedWeapon = GetMostValuableWeapon(Var(1), 1);
  if(!mostValuedWeapon)
    mostValuedWeapon = GetMostValuableWeapon(ShortRangeWeapon()|MiddleRangeWeapon()|LongRangeWeapon(), 1);
  if(!mostValuedWeapon)
  mostValuedWeapon = GetMostValuableWeapon(Var(1), 0);
    if(!mostValuedWeapon)
  mostValuedWeapon = GetMostValuableWeapon(ShortRangeWeapon()|MiddleRangeWeapon()|LongRangeWeapon(), 0);

  return(weapon=mostValuedWeapon);

private func GetMostValuableWeapon(int range, bool needsAmmo) {
  var mostValuedWeapon;
  while(Var(0)=Contents(Var(2)++)) {
    if(ObjectCall(Var(0), "IsWeapon")&range||(range==ShortRangeWeapon()&&ObjectCall(Var(0), "IsThrowWeapon"))) {
      if(Var(0)->~NeedsAmmo()&&needsAmmo) {
        if(Var(0)->~Ammo()>0) {
          if(!mostValuedWeapon||(mostValuedWeapon&&GetValue(mostValuedWeapon)<GetValue(Var(0))))
            mostValuedWeapon = Var(0);
        } else if(Var(0)->~ExtractAmmoPacket()!=-1) {
          if(!mostValuedWeapon||(mostValuedWeapon&&GetValue(mostValuedWeapon)<GetValue(Var(0))))
            mostValuedWeapon = Var(0);
        }
      } else if(!(Var(0)->~NeedsAmmo())&&!needsAmmo) {
        if(!mostValuedWeapon||(mostValuedWeapon&&GetValue(mostValuedWeapon)<GetValue(Var(0))))
          mostValuedWeapon = Var(0);
      }
    }
...
private AttackEnemy:
  ...
  if(weapon) Help1(weapon);

  if(weapon->~IsFiring()) {
    if(!target)
      weapon->~Fire();
    else if(!Inside(GetY(target),GetY()-30,GetY()+10)||!PathFree(GetX(),GetY(),GetX(target),GetY(target)))
      weapon->~Fire();
  } else StraightStrike();
  return(1);
...
private StraightStrike:
  if ( GreaterThan( Abs(Sub(GetX(),GetX(target))), 450) )
    return( 1 );  // Ziel zu weit weg: Hinlaufen
  if ( Abs(Sub(GetX(),GetX(target))) < 50 ) {
    SetComDir(COMD_None());
    if(Random(2))
      Jump();
    AddCommand(this(), "Wait", 0,0,0,0,50);
  }
  if ( LessThan( Abs(Sub(GetX(),GetX(target))), 10) )
    return(TooClose());  // Ziel zu nah: wegspringen
  if(Inside(GetY(target),Sub(GetY(),10),Sum(GetY(),10)))
...
private WaySearch:
  ...
  while(SetVar(1,FindObject(_MH1,-150,-150,300,300,0,0,0,0,Var(1) )) ) 
    ...
    if(ObjectDistance(Local(0,Var(1)),Var(0) ) + ObjectDistance(Var(0)) < ObjectDistance(Var(0))) {
      AddCommand(this(),"Wait",0,0,0,0,50);
  ...
  while(SetVar(1,FindObject(_HAC,-100,-100,200,200,0,0,0,0,Var(1) )) )
    if(Not(FindMaterial("DuroLava",GetX(Var(1)),Sum(GetY(Var(1)),200) )))
      if(Not(FindMaterial("Lava",GetX(Var(1)),Sum(GetY(Var(1)),200) )))
        if(Not(FindMaterial("Acid",GetX(Var(1)),Sum(GetY(Var(1)),200) )))
