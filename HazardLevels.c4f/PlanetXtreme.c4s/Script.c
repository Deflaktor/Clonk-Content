...
Initialize:
  ...
  SetGlobal(648,1);
  ScriptGo(1);
  if(Equal(GetPlayerCount(),1)) SetPosition(Random(LandscapeWidth()),0,CreateObject(KI58));
  return(0);
...
Script1:
  //GetPlayerCount *g*  <- Alt
  //SetGlobal(0,Sum(ObjectCount(CLON),ObjectCount(CLSH)));   
  
  //Barbar erstellen (bei der Final nur im Einzelspieler)
  //  if(Not(Random(Mul(GetPlayerCount(),100)))) SetPosition(Random(LandscapeWidth()),0,CreateObject(KI58));
  //if(Not(Random(100))) if(Equal(GetPlayerCount(),1)) SetPosition(Random(LandscapeWidth()),0,CreateObject(KI58));
  //Magisches Schutzfeld..
  //if(Not(Random(34))) CastObjects(_PRK,2,20,Random(LandscapeWidth()),0);
