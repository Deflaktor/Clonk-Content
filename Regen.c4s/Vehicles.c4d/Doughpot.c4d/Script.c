...
public func ProductionOrder(id defProduct, object pWorker)
{
  ...
  if (!ContentsCount(WBRL)) ProductionAcquireMaterial(WBRL,1,pWorker);
  // Ok
  return(1);
}
...
public func ProductionStart(clonk, def) 
{
  var obj;
  // Wasser suchen
  if (!(obj = FindContents(WBRL))) { Message("$TxtNeedsWater$", this); return false; }
  ...
  empty_barrel = CreateContents(BARL);
  // Clonk einsaugen
  Enter(this, clonk);
